...
# 前端传递的参数:地址的id(addr_id)支付方式(pay_method)用户购买商品的id字符串(sku_ids)
# MySQL事务:一组sql操作要么成功,要么失败
# 高并发:秒杀
# 支付宝支付
...
class CommentView(LoginRequiredMixin, View):
    ...
    def get(self, request, order_id):
        ...
        return render(request, 'order_comment.html', context=context)
    ...
    def post(self, request, order_id):
        ...
        for i in range(1, total_count + 1):
            # 获取评论的商品的id
            sku_id = request.POST.get('sku_{}'.format(i))  # sku_1 sku_2
            # 获取评论的商品的内容
            content = request.POST.get('content_{}'.format(i))  # content_1 content_2 content_3
            try:
